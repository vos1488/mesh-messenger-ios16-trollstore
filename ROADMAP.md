...
- [ ] Убрать редкие race-condition в call signaling при фоне/форграунде
- [ ] Дожать стабильность `MCStreamCallEngine` под длительные звонки (30+ мин)
- [ ] Закрыть edge-cases дублей чатов после перезапуска узла
- [ ] Доработать дедупликацию peer-списка на уровне памяти и SQLite
- [ ] Единая логика retry/backoff для исходящих сообщений и файлов
- [ ] Улучшить обработку ACK/read-receipt при временной потере маршрута
- [ ] Добавить recovery outbox после crash/kill процесса
- [ ] Унифицировать ошибки сети и крипто-ошибки в UI-диагностике
- [ ] Добавить ограничение роста in-memory буферов и очередей
- [ ] Оптимизировать cold start и старт discovery без пиков CPU
- [ ] Добавить smoke-check при старте (база, ключи, транспорт, update-check)
- [ ] Протестировать стабильность на iOS 16.0–16.7 и iOS 17/18

## Stage 2 — Messaging UX & chat management (v1.4)

- [ ] Ввести чат-настройки: mute, pin, archive, mark unread
- [ ] Добавить массовую очистку: «Очистить все чаты»
